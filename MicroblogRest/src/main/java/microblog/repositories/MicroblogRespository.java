...
public interface MicroblogRespository extends JpaRepository<Post, Integer> {
    ...
    // custom query to search the microblog post by title or content
    List<Post> findByTitleContainingOrContentContaining(String text, String textAgain);
    
    @Query("SELECT p FROM Post p JOIN p.postRatings r GROUP BY p ORDER BY p.dateTime DESC, SUM(r.value) DESC")
